...
export function PaymentStatusTable({ payments, className }: PaymentStatusTableProps) {
  ...
  useEffect(() => {
    ...
    const amounts = payments.reduce((acc, payment) => {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      completed: { variant: 'default' as const, label: '完了', className: 'bg-green-100 text-green-800' },
      pending: { variant: 'secondary' as const, label: '保留中', className: 'bg-yellow-100 text-yellow-800' },
      processing: { variant: 'secondary' as const, label: '処理中', className: 'bg-blue-100 text-blue-800' },
      failed: { variant: 'destructive' as const, label: '失敗', className: 'bg-red-100 text-red-800' },
      cancelled: { variant: 'destructive' as const, label: 'キャンセル', className: 'bg-gray-100 text-gray-800' },
      refunded: { variant: 'secondary' as const, label: '返金済み', className: 'bg-purple-100 text-purple-800' }
    }
  ...
  return (
    <div className={cn('space-y-4', className)}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="text-center p-3 bg-green-50 rounded-lg">
          <div className="text-lg font-bold text-green-800">¥{totals.completedAmount.toLocaleString()}</div>
          <div className="text-sm text-green-600">完了取引額</div>
        </div>
        <div className="text-center p-3 bg-purple-50 rounded-lg">
          <div className="text-lg font-bold text-purple-800">¥{totals.refundedAmount.toLocaleString()}</div>
          <div className="text-sm text-purple-600">返金額</div>
        </div>
        <div className="text-center p-3 bg-blue-50 rounded-lg">
          <div className="text-lg font-bold text-blue-800">
            {totals.totalTransactions > 0 ? Math.round(totals.completedAmount / totals.totalTransactions).toLocaleString() : 0}
          </div>
          ...
          <TableBody>
            {payments.map((payment) => (
              <TableRow key={payment.id}>
                ...
                <TableCell className="font-mono text-sm">{payment.reservationId.slice(-8)}</TableCell>
                ...
                <TableCell>
                  {payment.processedAt ? new Date(payment.processedAt).toLocaleString('ja-JP') : '-'}
                </TableCell>
              </TableRow>
            ))}
            {payments.length === 0 && (
