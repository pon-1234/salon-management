...
export class PricingUseCases {
  ...
  async createCourse(course: Omit<CoursePrice, 'id' | 'createdAt' | 'updatedAt'>): Promise<CoursePrice> {
  ...
  async createOption(option: Omit<OptionPrice, 'id' | 'createdAt' | 'updatedAt'>): Promise<OptionPrice> {
  ...
  async createAdditionalFee(fee: Omit<AdditionalFee, 'id' | 'createdAt' | 'updatedAt'>): Promise<AdditionalFee> {
  ...
  async calculateTotalPrice(
    ...
    if (isLateNight) {
      ...
      if (lateNightFee && lateNightFee.type === 'percentage' && typeof lateNightFee.value === 'number') {
  ...
  private determineOptionCategory(name: string): 'relaxation' | 'body-care' | 'extension' | 'special' {
