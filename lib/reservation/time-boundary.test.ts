...
describe('reservation start-time boundary', () => {
  it('accepts only exact 10-minute instants', () => {
    ...
    expect(isReservationStartBoundary(new Date('2099-01-01T12:05:00.000Z'))).toBe(false)
  ...
  it('rounds minute counts up to the next boundary without moving aligned values', () => {
    expect(ceilReservationStartMinutes(9 * 60 + 4)).toBe(9 * 60 + 10)
