/**
 * @design_doc   DEVELOPMENT_GUIDE.md public storefront server data boundary
 * @related_to   app/api/public/stores/[slug]/route.ts and public-api.ts share this payload builder
 * @known_issues docs/LEGACY_GOLD_ADMIN_MIGRATION_INVENTORY.md tracks legacy storefront data gaps
 */
...
import type { PublicCastSummary, PublicStoreHomeData } from './public-types'
...
function normalizeOpeningHours(storeSettings: any, fallback?: Store) {
...
function mapCastToSummary(cast: any): PublicCastSummary {
...
export async function loadPublicStoreHomeData(slug: string): Promise<PublicStoreHomeData | null> {
  const normalizedSlug = slug?.trim().toLowerCase()
  if (!normalizedSlug) {
    return null
  }

  const fallbackPayload = env.featureFlags.useMockFallbacks
    ? buildFallbackHomeData(normalizedSlug)
    : null
  ...
  try {
    ...
    if (!storeRecord || !storeRecord.isActive) {
      return fallbackPayload
    }

    const fallbackStore = env.featureFlags.useMockFallbacks
      ? getFallbackStoreBySlug(normalizedSlug)
      : undefined
    const openingHours = normalizeOpeningHours(storeRecord.storeSettings, fallbackStore)
    const store: Store = {
      id: storeRecord.id,
      ...
      theme: fallbackStore?.theme,
      seoTitle: fallbackStore?.seoTitle,
      seoDescription: fallbackStore?.seoDescription,
    ...
    const newcomers = uniqueBy(newcomersRaw.map(mapCastToSummary)).slice(0, 4)
