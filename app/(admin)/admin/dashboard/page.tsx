...
// KPIカードコンポーネント
function KPICard({ title, value, change, changeLabel, icon, trend, color = 'primary', sparklineData }: KPICardProps) {
  const trendColors = {
    up: 'text-green-600',
    down: 'text-red-600',
      ...
      <CardContent className="p-6">
        <div className="flex items-start justify-between mb-4">
          <div className={cn("p-3 rounded-lg", bgColors[color])}>
            <div className={cn("w-5 h-5", iconColors[color])}>
              {icon}
            </div>
          </div>
          {trend && (
...
function StatusBadge({ status }: { status: string }) {
  const statusConfig = {
    confirmed: { label: '確定', variant: 'default' as const, icon: <CheckCircle2 className="w-3 h-3" /> },
    pending: { label: '保留中', variant: 'secondary' as const, icon: <Clock className="w-3 h-3" /> },
    cancelled: { label: 'キャンセル', variant: 'destructive' as const, icon: <XCircle className="w-3 h-3" /> },
    completed: { label: '完了', variant: 'outline' as const, icon: <CheckCircle2 className="w-3 h-3" /> },
    modifiable: { label: '修正可能', variant: 'outline' as const, icon: <AlertCircle className="w-3 h-3" /> }
  }
...
export default function DashboardPage() {
  ...
  const generateSalesData = () => {
    const last7Days = Array.from({ length: 7 }, (_, i) => {
      ...
      const dayReservations = reservations.filter(r => 
        format(r.startTime, 'yyyy-MM-dd') === format(date, 'yyyy-MM-dd')
      )
      return {
  ...
  const statusDistribution = [
    { name: '確定済み', value: reservations.filter(r => r.status === 'confirmed').length, color: colors.success },
    { name: '保留中', value: reservations.filter(r => r.status === 'pending').length, color: colors.warning },
    { name: 'キャンセル', value: reservations.filter(r => r.status === 'cancelled').length, color: colors.danger },
    { name: '完了', value: reservations.filter(r => r.status === 'completed').length, color: colors.info }
  ]
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KPICard
          ...
          change={kpis.previousBookings > 0 ? Math.round(((kpis.totalBookings - kpis.previousBookings) / kpis.previousBookings) * 100) : 0}
          changeLabel={`${kpis.confirmedCount}件確定済み`}
          icon={<Calendar />}
          trend={kpis.totalBookings > kpis.previousBookings ? 'up' : kpis.totalBookings < kpis.previousBookings ? 'down' : 'neutral'}
          color="success"
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {format(reservation.startTime, 'MM月dd日 HH:mm')} - {format(reservation.endTime, 'HH:mm')}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
