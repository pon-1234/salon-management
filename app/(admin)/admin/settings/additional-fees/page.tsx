...
export default function AdditionalFeesPage() {
  ...
  const handleSaveFee = async () => {
    try {
      const value = formData.type === 'range' 
      ...
      if (editingFee) {
        ...
        setFees(prev => prev.map(fee => 
          fee.id === editingFee.id ? updated : fee
        ))
        toast({
          title: "更新完了",
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      case 'fixed': return '固定額'
      case 'percentage': return 'パーセンテージ'
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => setFormData(prev => ({ ...prev, minValue: parseInt(e.target.value) || 0 }))}
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) => setFormData(prev => ({ ...prev, maxValue: parseInt(e.target.value) || 0 }))}
                      />
                    </div>
                  </div>
                )}
