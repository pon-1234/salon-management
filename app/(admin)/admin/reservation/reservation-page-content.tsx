...
export function ReservationPageContent() {
  ...
  const handleReservationSave = async (
    ...
    try {
      ...
      if (payload.areaId !== undefined) {
        const oldAreaId = targetReservation.areaId ?? null
        const newAreaId = payload.areaId ?? null
        if (normalizeToken(oldAreaId) !== normalizeToken(newAreaId)) {
          const oldAreaLabel =
            (targetReservation as any).areaName ?? oldAreaId ?? '未設定'
          const newAreaLabel =
            refreshed?.areaName ?? newAreaId ?? '未設定'
          logChange('areaId', 'エリア', oldAreaLabel, newAreaLabel, '対応エリアを更新')
        }
      }

      if (payload.stationId !== undefined) {
        const oldStationId = targetReservation.stationId ?? null
        const newStationId = payload.stationId ?? null
        if (normalizeToken(oldStationId) !== normalizeToken(newStationId)) {
          const oldStationLabel =
            (targetReservation as any).stationName ?? oldStationId ?? '未設定'
          const newStationLabel =
            refreshed?.stationName ?? newStationId ?? '未設定'
