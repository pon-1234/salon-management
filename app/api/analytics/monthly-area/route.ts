...
import { getMonthlyAreaSummary } from '@/lib/analytics/server'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const report = await getMonthlyAreaSummary(year, month, storeId)
    return NextResponse.json(report, { status: 200 })
  } catch (error) {
    console.error('[analytics.monthly-area] failed to build report', error)
