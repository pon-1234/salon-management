...
import { getStaffAttendanceReport } from '@/lib/analytics/server'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const report = await getStaffAttendanceReport(year, month, storeId)
    return NextResponse.json(report, { status: 200 })
  } catch (error) {
    console.error('[analytics.staff-attendance] failed to build report', error)
