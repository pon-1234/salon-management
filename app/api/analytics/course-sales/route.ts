...
import { getCourseSalesReport } from '@/lib/analytics/server'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const report = await getCourseSalesReport(year, month, storeId)
    return NextResponse.json(report, { status: 200 })
  } catch (error) {
    console.error('[analytics.course-sales] failed to build report', error)
