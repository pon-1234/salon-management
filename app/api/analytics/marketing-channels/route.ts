...
import { getMarketingChannelReport } from '@/lib/analytics/server'
...
export async function GET(request: NextRequest) {
  try {
    ...
    const report = await getMarketingChannelReport(year, storeId)
    return NextResponse.json(report, { status: 200 })
  } catch (error) {
    console.error('[analytics.marketing-channels] failed to build report', error)
