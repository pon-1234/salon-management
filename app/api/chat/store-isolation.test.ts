 ...
 * @known_issues Customer identities are scoped through the approved compatibility policy
 */
...
describe('administrative chat store isolation', () => {
  ...
  it('lists only casts belonging to the selected authorized store', async () => {
    ...
    expect(requireAdmin).toHaveBeenCalledWith({ storeId: 'store-a' })
  ...
  it('rejects a customer thread without reservation-derived membership in a multi-store setup', async () => {
    vi.mocked(db.store.count).mockResolvedValue(2)
  ...
  it('updates read status only for an incoming message visible in the selected store', async () => {
    ...
    expect(db.message.findFirst).toHaveBeenCalledWith({
      where: {
        ...
        OR: [{ cast: { is: { storeId: 'store-a' } } }, { customer: { is: {} } }],
      },
      select: { id: true },
    })
  ...
  it('uses reservation-derived customer membership when multiple stores are active', async () => {
    vi.mocked(db.store.count).mockResolvedValue(2)
    ...
    expect(db.customer.findMany).toHaveBeenCalledWith({
      where: { reservations: { some: { storeId: 'store-a' } } },
      select: { id: true },
    })
  })
