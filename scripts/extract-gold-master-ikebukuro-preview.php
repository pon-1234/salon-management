...
function runExtractor()
{
    ...
    try {
        ...
        if ($extractKind !== 'gold-master-v4' && $extractKind !== 'cast-ledger' && $extractKind !== 'cast-profiles') {
        ...
        } elseif ($extractKind !== 'cast-profiles') {
        ...
        if ($extractKind === 'cast-profiles') {
            $stage = 'profile-refresh';
            $casts = $pdo->query("SELECT g.girl_no, g.shop_no, g.lev, g.name, g.age, g.tel, g.mail_ad, g.pref_flg, g.birth, g.regist_date, g.quit_day, g.blog_widget, g.twitter_id, g.p_height, g.p_bust, g.p_bust_cup, g.p_waist, g.p_hip, g.p_type, g.p_type2, g.p_play, g.search_1, g.search_2, g.search_2_text, g.home_flg, g.tattoo_flg, g.blood_flg, g.foreigner_flg, g.profile_catch, g.profile_cm, g.check_3, g.check_3_day, g.check_4, g.check_4_day, g.options, g.options_free, g.profile_new_1, g.profile_new_2, g.profile_new_3, g.profile_new_4, g.profile_new_5, g.profile_new_6, g.profile_new_7, g.profile_new_8, g.photo_1, g.photo_2, g.photo_3, g.photo_4, g.photo_5, g.photo_6, g.photo_7, g.photo_8, g.photo_9, g.photo_10, g.photo_11, g.photo_12, g.photo_13, g.photo_14, g.photo_15, u.name AS interviewer_name, m.media_name AS recruitment_name FROM girls g LEFT JOIN users u ON u.user_id = g.check_1_sel LEFT JOIN media m ON m.serial = g.media WHERE g.shop_no = 5600 AND g.lev IN (2, 3) AND g.lev_admin != 1 ORDER BY g.girl_no")->fetchAll();
            $media = $pdo->query("SELECT m.serial, m.kind, m.media_name, m.media_id, m.media_pw, m.media_url, m.display_url, m.memo, m.lev FROM media m WHERE m.pref_no = (SELECT pref_no FROM shop_list WHERE shop_no = 5600) ORDER BY m.kind, m.sort, m.serial")->fetchAll();
            $json = json_encode(array('kind' => 'cast-profiles', 'shopNo' => 5600, 'capturedAt' => $capturedAt, 'casts' => $casts, 'media' => $media), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
            if (!is_string($json)) throw new RuntimeException('Invalid profile encoding.');
            echo $json . "\n";
            $pdo->exec('COMMIT');
            $transactionStarted = false;
            return;
